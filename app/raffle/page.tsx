...
export default function RafflePage() {
  // Supabase generated types in this project currently resolve some tables
  // to never[]. Using a local any cast prevents those incorrect TypeScript
  // errors without changing the database.
  ...
  async function verifyPhone() {
    ...
    if (!isValidPhone(phone)) {
      setPhoneMsg({
        text: "شماره موبایل معتبر نیست (مثلاً 09123456789)",
        type: "err",
      });
    ...
    if (fetchError) {
      ...
      setPhoneMsg({
        text: "مشکلی پیش اومد، دوباره امتحان کن",
        type: "err",
      });
    ...
    if (insertError || !created) {
      ...
      setPhoneMsg({
        text: "مشکلی در ثبت شماره پیش اومد، دوباره امتحان کن",
        type: "err",
      });
    ...
    /*
     * Credit the person who invited this participant.
     */
    ...
    if (refCode) {
      ...
      if (
        ...
        const newAllowedSpins =
          Number(referrer.spins_allowed || 0) + 1;

        await supabase
          .from("raffle_participants")
          .update({
            spins_allowed: newAllowedSpins,
          })
          .eq("id", referrer.id);

        /*
         * Save who referred this participant.
         */
        await supabase
          .from("raffle_participants")
          .update({
    ...
    /*
     * Reload the newly created participant in case
     * referred_by was updated.
     */

    const {
      data: finalParticipant,
    } = await supabase
      .from("raffle_participants")
      .select("*")
      .eq("id", created.id)
    ...
    setParticipant(
      (finalParticipant || created) as Participant
    );

    setPhoneMsg({
      text: "خوش اومدی! ۲ چرخش رایگان داری.",
      type: "ok",
    });
  ...
  async function doSpin() {
    ...
    /*
     * Refresh segments so prize availability reflects
     * other players' spins.
     */
    ...
    const currentSegments =
      (freshSegments ?? segments) as Segment[];
    ...
    setTimeout(async () => {
      ...
      /*
       * Prize availability is checked again on the server.
       * This prevents two users from winning the same prize.
       */
    ...
    >
      ...
      <div className="text-center">
        ...
        <p className="mt-1 text-[11px] text-[#8A968C]">
          ۲ چرخش رایگان برای هر شماره + چرخش
          اضافه با دعوت دوستان
        </p>
      </div>
      ...
      {!segments ? (
        ...
        <div className="rounded-[20px] border border-[#E3EBDE] bg-white p-6 text-center text-sm text-[#8A968C]">
          هنوز بخش‌های قرعه‌کشی تنظیم نشده‌اند.
        </div>
      ) : (
        <>
      ...
      <p className="text-center text-[10px] leading-6 text-[#B0BAB1]">
        توجه: شماره موبایل و نتیجه‌ی چرخش‌ها
        روی سرور مشترک ذخیره می‌شه تا از
        چرخش بیشتر از حد مجاز هر شماره
        جلوگیری بشه. تاریخچه با شماره‌ی
        نصفه‌مخفی به همه نمایش داده می‌شه.
      </p>
    </div>
  );
}