...
export default function HomePage() {
  ...
  return (
    <div dir="rtl" className="home-shell space-y-1 rounded-[30px] pb-8 pt-0 sm:space-y-3 sm:pb-10">
      ...
      {/* GOLD BUSINESSES — فقط زیر نقشه، بدون چرخش خودکار */}
          ...
          <div className="space-y-2.5">
            {goldProductGroups.map(({ business, products: groupProducts }) => (
              <div key={business.id} className="flex min-w-0 items-stretch gap-2.5 rounded-2xl border border-[#F0DDB2] bg-white/70 p-2" dir="rtl">
                <Link href={`/business/${business.id}`} className="flex w-[92px] shrink-0 flex-col items-center justify-center gap-1 rounded-xl border border-[#F0D27F] bg-gradient-to-b from-[#FFF8DF] to-[#FFF0C4] px-1.5 py-2 text-center transition hover:shadow-[0_8px_18px_rgba(180,119,21,.16)] sm:w-[132px] sm:px-2.5">
