  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-slate-50">
      <div className="fade-in mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
            {businesses.map((b) => {
              ...
              return (
                <button
                  key={b.id}
                  onClick={() => b.subscription_status === "approved" ? router.push(`/business/manage/${b.id}/products`) : setActiveId(b.id)}
        ...
        {active && active.subscription_status === "approved" && (
          <div className="rounded-[24px] border border-emerald-200 bg-gradient-to-l from-emerald-50 via-white to-amber-50 p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-base font-black text-slate-800">📋 منو و محصولات {active.name}</h2>
                <p className="mt-1 text-[10px] text-slate-500">برای ثبت، ویرایش و مدیریت منو و محصولات وارد صفحه اختصاصی کسب‌وکار شوید.</p>
              </div>
              <button
                type="button"
                onClick={() => router.push(`/business/manage/${active.id}/products`)}
                className="rounded-xl2 bg-gradient-to-l from-jam-green to-emerald-400 px-5 py-3 text-xs font-black text-white shadow-glow"
              >
